...
Feature: Symfony Integration
  Scenario: Successfully access menu
    ...
    And the JSON node "root[0].subMenus[0]" should exist
    And the JSON node "root[0].subMenus[1]" should not exist
  ...
  Scenario: menu for logged in user
    ...
    Then the response status code should be 200
    And the response should be in JSON
    And the JSON node "root[0].subMenus[1]" should exist
    And the JSON node "root[0].subMenus[2]" should not exist
  ...
  Scenario: menu for logged in admin
    ...
    And the response should be in JSON
    And the JSON node "root[0].subMenus[1]" should exist
    And the JSON node "root[0].subMenus[2]" should exist
